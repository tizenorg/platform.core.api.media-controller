...
gboolean _mc_read_service_request_tcp_socket(GIOChannel *src, GIOCondition condition, gpointer data)
{
	...
	req = (mc_service_request *) malloc(sizeof(mc_service_request));
	if (req == NULL) {
		send_msg = MEDIA_CONTROLLER_ERROR_OUT_OF_MEMORY;
		goto ERROR;
	}
	memset(req, 0x00, sizeof(mc_service_request));
	...
	if (recv_msg.msg_type >= 0 && recv_msg.msg_type < MC_MSG_MAX) {
		...
		if (!g_queue_work) {
			...
			g_source_set_callback(src_request, _mc_service_process, mc_svc_data, NULL);
...
gboolean mc_svc_thread(void *data)
{
	...
	mc_svc_data = (mc_svc_data_t *)malloc(sizeof(mc_svc_data_t));
	if (mc_svc_data == NULL) {
